...
#1. Visual On Vriable `Trip Distance`
...
Before we move onto next step: prediction model building, we need to clean the data first, eliminate less responsible data and remove observations that may looks odd.
...
y = y[y$longitude < 0 & y$latitude > 0, ]
...
sqr = lm(Tip_ratio ~.^2, data = nofactor)
BIC2 <- step(sqr, direction = "backward", trace = 0)
summary(BIC2)
```

`BIC2` is a better model with R-squared `1`

```{r}

lmtest::bptest(BIC2) #p-val:  2.2e-16
set.seed(110116) 
resid5000 = sample(resid(BIC2), 5000)
shapiro.test(resid5000) #p-value: 2.2e-16

hat = hatvalues(BIC2); hat_bar = mean(hat); i = which(hat > 2 * hat_bar)

n = which(abs(rstandard(BIC2)) > 1)
index = which(cooks.distance(BIC2) > 4 / length(cooks.distance(BIC2)))

y1 = nofactor[-i,]
y2 = y1[-index,]
y3 = y2[-n,]
y3$Tip_ratio = y3$Tip_ratio + 0.001
all2 = lm(Tip_ratio ~., data = y3)
plot(all2, pch=20, cex=0.3, col='dodgerblue')
trans = lm(I(Tip_ratio)^2 ~.^2, data = y3)
BIC3 <- step(trans, direction = "backward", trace = 0)
summary(BIC3)
```
